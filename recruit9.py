...
with st.sidebar:
    ...
    # if get_platform in ["all", "saramin", "wanted"]:
    # # 주소창 값이 있으면 그에 맞는 인덱스 설정 (공유 기능 유지)
    #     if get_platform == "saramin": p_index = 1
    #     elif get_platform == "wanted": p_index = 2
    #     else: p_index = 0
    # else:
...
if search_submit or auto_search:
    if keyword:
        ...
        if search_submit:
            st.cache_data.clear()
            st.query_params.update(kw=keyword, platform=platform_map_to_id.get(platform_choice, "saramin"))
        ...
        # 백그라운드 데이터 수집 스레드 설정
        res_df_container = []
        def background_scraping(target_kw, count):
            try:
                # 실제 수집 함수 호출
                data = get_saramin_jobs(keyword, row_count)
                res_df_container.append(data)
            except Exception as e:
                res_df_container.append(None)

        # 스레드 시작
        scraping_thread = threading.Thread(target=background_scraping,args=(keyword, row_count))
        scraping_thread.start()

        # 무중단 멘트 로테이션 (UI 루프)
        idx = 0
        with spinner_placeholder:
            with st.spinner(" "): # 돌기 시작
                while scraping_thread.is_alive():
                    msg = engine_logs[idx % len(engine_logs)]
                    # 멘트를 스피너 바로 옆에 렌더링
                    log_placeholder.markdown(f"**{msg}**")
                    
                    time.sleep(0.9) # 전환 속도
                    idx += 1
        
        # 수집 완료 후 스레드 합류 및 UI 정리
        scraping_thread.join()
        log_placeholder.empty()
        spinner_placeholder.empty()

        # 결과 처리 및 리런
        res_df = res_df_container[0] if res_df_container else None
