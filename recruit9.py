...
def analyze_with_llama(crawled_text, image_urls=None):
    ...
    ###1. 공고 기초 정보 
    - **공고 기초 정보:** 회사명, 공고 제목, 근무지, 자격 요건(최소 요구 연차, 학력 위주 2개 이하만 표기할것), 제출 기한
    ...
    ---
    [출력 규칙] 표 금지, 불렛포인트 사용, 기본적으로 한국어(한글)사용, 한자(중국의 문자; 번체자, 간체자)일체 사용(표기) 금지, 일본어(일본의 문자; 히라가나, 카타카나) 일체 사용(표기)금지.
